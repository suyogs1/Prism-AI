/* Mock data for demo */
const getTodayIso = (daysAgo = 0, hours = 9, mins = 30) => {
  const d = new Date();
...
const getTodayDateStr = () => new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
export const DEMO_APPLICATIONS = [
  {
    ...
    submittedAt: getTodayIso(0, 9, 30),
    subScores: {
      gst: 81,
      cashflow: 72,
      stability: 78,
...
`,
  },
  {
    ...
    confidence: 'MEDIUM',
    status: 'UNDER_REVIEW',
    submittedAt: getTodayIso(1, 14, 15),
    subScores: {
      gst: 62,
      cashflow: 54,
      stability: 65,
...
The deterministic engine has computed a confidence band of MEDIUM due to absence of ITR and UPI transaction data. The recommended loan ceiling of ₹3.8 lakhs reflects conservative capacity sizing pending supplementary data submission.`,
  },
  {
    ...
    submittedAt: getTodayIso(2, 11, 0),
    subScores: {
      gst: 38,
      cashflow: 44,
      stability: 52,
...
All five scoring indices return above-threshold values. The deterministic engine has computed a recommended loan amount of ₹7.5 lakhs based on a 4x monthly cash inflow multiple, adjusted upward by the HIGH confidence multiplier.`,
  },
  {
    ...
    confidence: 'MEDIUM',
    status: 'PENDING_REVIEW',
    submittedAt: getTodayIso(4, 16, 45),
    subScores: {
      gst: 60,
      cashflow: 66,
      stability: 70,
...
The digital footprint index of 42 is the primary score limiter, reflecting limited online presence. The engine has computed a recommended loan of ₹5.2 lakhs. Submission of UPI transaction history would improve confidence from MEDIUM to HIGH.`,
  },
  {
    ...
    confidence: 'HIGH',
    status: 'APPROVED',
    submittedAt: getTodayIso(5, 14, 20),
    subScores: {
      gst: 84,
      cashflow: 79,
      stability: 75,
...
GST filing regularity is pristine over the assessment window. The deterministic engine computed an approved funding ceiling of ₹14 lakhs based on debt service capacity and business age multipliers.`,
  },
  {
    ...
    confidence: 'MEDIUM',
    status: 'UNDER_REVIEW',
    submittedAt: getTodayIso(6, 11, 15),
    subScores: {
      gst: 55,
      cashflow: 74,
      stability: 72,
...
export const MSME_DEMO = {
  ...
  completeness: 82,
  documents: [
    { name: 'GST Returns (12 months)', status: 'verified', uploadedAt: getTodayYmd() },
    { name: 'Bank Statement (12 months)', status: 'verified', uploadedAt: getTodayYmd() },
    { name: 'Udyam Certificate', status: 'verified', uploadedAt: getTodayYmd() },
    { name: 'ITR (2 years)', status: 'pending', uploadedAt: null },
    { name: 'UPI Transaction History', status: 'missing', uploadedAt: null },
  ],
  timeline: [
    { event: 'Application Submitted', date: getTodayDateStr(), done: true },
    { event: 'Document Verification', date: getTodayDateStr(), done: true },
    { event: 'Prism Score Computed', date: getTodayDateStr(), done: true },
