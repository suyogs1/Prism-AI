...
export function evaluateProfile(profile, configOverride = null) {
  ...
  const cash_flow = computeCashFlow(profile)
  const growth = computeGrowth(profile)
  const trust = computeTrust(profile)
  const { risk, risk_flags } = computeRiskSafety(profile)
  ...
  if (recommendation !== "DECLINE") {
    const rev = parseFloat(profile.monthly_revenue || 0)
    const emi = parseFloat(profile.existing_loans_monthly_emi || 0)
    ...
    const requested = parseFloat(profile.requested_loan_amount || 0)
    if (requested > 0) {
...
function computeConfidence(profile, cfg = CONFIG) {
  ...
  if (profile.bank_statements_present) {
  ...
  if (profile.relationship_present) {
...
function computeCashFlow(profile) {
  ...
  const avg_bal = parseFloat(profile.average_bank_balance || 0)
  const emi = parseFloat(profile.existing_loans_monthly_emi || 0)
...
function computeTrust(profile) {
  const age = parseFloat(profile.business_age || 0)
  const age_score = age >= 5.0 ? 100.0 : age >= 3.0 ? 85.0 : age >= 1.0 ? 60.0 : 30.0

  let compliance = 100.0
  if (!profile.epfo_regular) compliance -= 30.0
  if (!profile.gst_regular) compliance -= 30.0
...
function computeRiskSafety(profile) {
  ...
  const bounce_rate = parseFloat(profile.bounce_rate || 0)
  const existing_loans = parseFloat(profile.existing_loans_balance || 0)
  const turnover = parseFloat(profile.turnover || 1.0)
